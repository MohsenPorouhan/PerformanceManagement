...
{
    public class SensibleEventService
    {
        private readonly AppDbContext appDbContext;
        private readonly IConnProvider connProvider;
        private readonly IHostingEnvironment hostingEnvironment;

        public SensibleEventService(AppDbContext appDbContext, IConnProvider connProvider, IHostingEnvironment hostingEnvironment)
        {
            this.appDbContext = appDbContext;
            this.connProvider = connProvider;
            this.hostingEnvironment = hostingEnvironment;
        }
        public IEnumerable<object> GetTaskListOfEmployee(int employeeDepartmentId, int employeeId, int coacherDepartmentId)
        {
            ...
            string sQuery = "select " +
                ...
                "where " +
                "1 = 1 " +
                "and e.RecieverAllocationEvaluationHierarchyId = @employeeDepartmentIdd " +
                "and e.RecieverAllocationPersonId = @employeeIdd " +
                "and(e.AllocatorEvaluationHierarchyId = @coacherDepartmentIdd or e.AllocatorEvaluationHierarchyId is null) " +
        ...
        public IEnumerable<object> GetCompetencyListOfEmployee(int employeeDepartmentId, int employeeId, int coacherDepartmentId)
        {
            ShareService shareService = new ShareService(appDbContext, null);
            int periodDefinitionId = shareService.GetPeriodDefinitionId();
            IDbConnection conn = connProvider.Connection;
            string sQuery = "select " +
                ...
                "where " +
                "1 = 1 " +
                "and ebc.RecieverAllocationEvaluationBehaviouralHierarchyId = @employeeDepartmentIdd " +
                "and ebc.RecieverAllocationPersonId = @employeeIdd " +
                "and(ebc.AllocatorEvaluationBehaviouralHierarchyId = @coacherDepartmentIdd or ebc.AllocatorEvaluationBehaviouralHierarchyId is null) " +
        ...
        public int AddSensibleEvent(string title, int eventType, DateTime sensibleEventDate, string[] behaviourCompetencyId, string[] taskId, bool visibility, string departmentSupervisor, string roleId, int personId, string subSetEmployeeId, IFormFile fupload, string description)
        {
            ShareService shareService = new ShareService(appDbContext, null);
            ...
            if (fupload != null)
            {
                string uploadsFolder = Path.Combine(hostingEnvironment.WebRootPath, "coacherUploads");
            ...
            sensibleEvent.Visibility = visibility;
            sensibleEvent.PersonId = personId;
            sensibleEvent.PersonDepartmentId = int.Parse(departmentSupervisor.Split("-")[0]);
            sensibleEvent.PersonRole = roleId;
            sensibleEvent.EmployeeId = int.Parse(subSetEmployeeId.Split("-")[1]);
            sensibleEvent.EmployeeDepartmentId = int.Parse(subSetEmployeeId.Split("-")[0]);
        ...
        public Dictionary<object, object> GetSensibleEventList(DataTableParameter dataTableParameter, int? coacherDepartmentId, int? employeeId, string roleId,int? periodDefinitionId)
        {
            String[] aColumns = { "Title", "Description", "EmployeeFullName", "EmployeeDepartmentName", "CoacherDepartmentName" };
            ...
            if (dataTableParameter.length != -1 && dataTableParameter.search.Equals(""))
            {
                query = conn.Query<object>(sQuery, new
                {
                    ...
                    personRolee = roleId,
                    personDepartmentIdd = coacherDepartmentId,
                    employeeIdd = employeeId,
                    periodDefinitionIdd= periodDefinitionId
                }).ToList();
            }
